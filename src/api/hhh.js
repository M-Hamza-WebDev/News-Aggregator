...
const NYTIMES_API = "https://api.nytimes.com/svc/search/v2/articlesearch.json?";
...
export const getNewsFromApi = async (query, filter, source) => {
  try {
    ...
    if (source === "nytimes") {
      console.log({ nytimes: response.data.response.docs });
      const nytimesNewsData = response.data.response.docs.map((_news) =>  {
        return {
          title: _news.snippet,
